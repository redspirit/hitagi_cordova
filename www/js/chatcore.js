...
function uplImage(file){
	...
	imageReader.onload = (function(aFile){
		return function(e){
			...
			imaga.src = e.target.result;
			imaga.onload = function(){
				ch.uploadImage(currentRoom, imaga.src);
			}
		}
	})(file);
	imageReader.readAsDataURL(file);
...
function getUserItemHTML(name, nick, avaurl, status, priv, state, pm){
	if(pm){
...
function addUser(room, name, uobj){
	...
	roomSel(room, '.rp').append(getUserItemHTML(name, uobj['nick'], uobj['avaurl'], uobj['statustext'], uobj['commonPriv'], uobj['state']));
...
function fillUsers(usr){
	for(var key in usr){
		curRoomSel('.rp').append(getUserItemHTML(key, usr[key]['nick'], usr[key]['avaurl'], usr[key]['statustext'], usr[key]['commonPriv'], usr[key]['state']));
	}
...
function showDesktopNotif(capt, mes, userpic){
	if(isNotif && notifEnable && !isFocus && webkitNotifications.checkPermission()==0){
		var nn = webkitNotifications.createNotification(userpic, capt, strip_tags(stripBB(mes)).substring(0,200));
		nn.show();
		nn.onclick = function(){
			this.cancel();
			curRoomSel('.messageinput').focus();
